# Database Security — Teknovo Security System

> **Scope**: UUID v7, soft delete, audit logs, ownership tracking, FK constraints, data isolation  
> **Aligns with**: `docs/standards/database/database-standard.md`, `.agents/skills/teknovo-database-architect/SKILL.md`  
...
PostgreSQL 17 holds authoritative school data. Access path is **Repository only** — no direct HTTP-to-DB, no cross-module repository imports. Database binds to `127.0.0.1`; Cloudflare Tunnel exposes API — not PostgreSQL.

```text
API Service → Repository (tenant-aware queries) → PostgreSQL (private)
                     ↓
...
### Protected Tables (No Hard Delete — Ever)
...
**Finance payment example**: `finance.payment.created` with bill id, amount, method — no full card data.
...
| Column | Usage |
|--------|-------|
| `school_id` | Multi-tenant isolation — **mandatory** on school-scoped tables |
| `created_by` | Accountability |
| Domain FKs | `student_id`, `class_id`, `applicant_id` with proper constraints |

### Tenancy Rule

Every list/read/write query in repository must include `school_id` from authenticated context — **never** from client-supplied body alone.
...
| Rule | Example |
|------|---------|
| FK with explicit on-delete policy | Soft delete parent → children remain or cascade soft |
| No orphan PII | Student delete (soft) → handle dependent records |
...
| Rule | Implementation |
|------|----------------|
| Parameterized only | Drizzle ORM — no string concat SQL |
...
- API layer: `security/api-security.md`
- RBAC: `security/rbac-security.md`
- Cloudflare R2 backups: `security/cloudflare-security.md`
- Skill: `teknovo-database-architect`
- Memory: `memory/architecture-decisions.md`
