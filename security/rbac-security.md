# RBAC Security — Teknovo Security System

> **Scope**: Role hierarchy, permission mapping, route guards, endpoint authorization, UI visibility  
> **Aligns with**: `docs/standards/rbac/rbac-standard.md`, `.agents/skills/teknovo-rbac-architect/SKILL.md`, `AGENTS.md`  
...
RBAC is the primary authorization control for Teknovo. Security treats **frontend-only hiding** as a vulnerability — backend guards are authoritative.
...
**Reject**: Hardcoded `if (role === 'ADMIN_SEKOLAH')` for feature access — use `hasPermission('domain.resource.action')`.
...
| `finance.payment.create` | POST payment — **high risk** |
| `cbt.exam.start` | Start attempt; SISWA only with ownership |
| `academic.grade.input` | GURU scoped to assigned classes |

Seed permissions in migrations; update `docs/security/rbac-matrix.md` on every change.
...
|-------|----------------|----------------------|
| 1. Menu visibility | Sidebar `v-if="hasPermission(...)"` | Information disclosure (low) — still fix |
| 2. Route access | Nuxt middleware / route meta | Direct URL access bypass |
| 3. API access | Controller `@RequirePermissions` | **Critical** — data mutation/theft |
| 4. Action access | Button visibility | UX confusion; pairs with layer 3 |
| 5. Data ownership | Service filters by `school_id`, class, student link | **Critical** — horizontal privilege escalation |

### Layer 5 — Data Ownership (Mandatory)
...
**Verify**: GET endpoints have guards — listing applicants without `ppdb.applicant.read` is a leak.
...
Show **Permission** page state when guard fails — never blank screen or silent redirect loop.
...
## PPDB Example — Verification Flow
...
Missing any layer → **BLOCK** in Security Review.

---

## Finance Example — Payment Creation

High-risk mutation — require:
...
| Wrong school context | Empty set or 403 — never other school rows |
...
- Principles: `security/security-principles.md`
- Checklist: `security/review-checklist.md` — Authorization section
