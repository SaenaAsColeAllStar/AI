...
> **Agent mapping**: `registry/agent-registry.yaml`  
> **MCP integrations**: `registry/mcp-registry.yaml`  
...
```text
                    ┌─────────────────────────┐
                    │  registry/skill-registry │  ← SINGLE SOURCE OF TRUTH
                    │  (layers, priorities,    │
                    │   activation, conflicts) │
                    └───────────┬─────────────┘
          ┌─────────────────────┼─────────────────────┐
          ▼                     ▼                     ▼
  .agents/registry.yaml   Sub-registries          registry/
  (legacy autoload)       taste/, quality/,       agent-registry.yaml
                          security/, memory/       mcp-registry.yaml
```

**Precedence for orchestration**: `registry/skill-registry.yaml` overrides informal references. `.agents/registry.yaml` remains for backward compatibility — it points to the canonical registry and must not diverge on autoload IDs.
...
| 4 | `registry/skill-registry.yaml` | Legacy registry entries |
| 5 | Teknovo Skills (`.agents/skills/teknovo-*`) | Generic AI defaults |
...
Every skill has **exactly one primary layer** (see `registry/skill-registry.yaml` → `layers`):
...
| **medium** | Trigger-based domain/taste/quality | Domain modules (PPDB, finance), taste principles |
...
| Sub-registry bundle | `taste/quality/security-registry.yaml` | `load-memory.py --taste-bundle` |
...
MCP integrations are tracked in `registry/mcp-registry.yaml`:
...
- **Teknovo skill**: `.agents/skills/teknovo-{name}/SKILL.md`
- **Superpowers/GStack**: `.agents/skills/{family}/{name}/SKILL.md`
...
Add entry under `registry/skill-registry.yaml` → `skills`:
...
```yaml
  teknovo-new-module:
    ...
    source: teknovo
    path: .agents/skills/teknovo-new-module/SKILL.md
...
Add to `registry/agent-registry.yaml` under `agents:` with `skills.required` / `recommended`.

### Step 5 — Legacy sync (minimal)

If autoload or commonly triggered, add ID to `.agents/registry.yaml` → `autoload` or appropriate phase section **or** rely on canonical pointer only for on-demand skills.

### Step 6 — Document and validate

1. Add row to `docs/ai/skill-inventory.md`
2. Run `python ai-agent/runtime/load-skills.py --validate`
3. Run `python ai-agent/runtime/load-skills.py --trigger "your keywords"`

### Step 7 — MCP (if external tool)

Register in `registry/mcp-registry.yaml` with `risk_level` and `required_permissions`.
...
2. Resolve autoload from `registry/skill-registry.yaml`
3. Match user intent → `--trigger` or agent `activate_when`
4. Load memory context: `python ai-agent/runtime/load-memory.py`
5. Load phase bundles as needed:
   ...
   - Skills: `python ai-agent/runtime/load-skills.py --bundle pre-implementation`

**Agent persona lookup**:

```text
registry/agent-registry.yaml → agents.{id}.skills → skill IDs → skill-registry.yaml → paths
```
...
| `assurance/assurance-registry.yaml` | Active — assurance bundles |
...
| Document | Purpose |
|----------|---------|
| `registry/README.md` | Registry architecture quick reference |
| `docs/ai/skill-inventory.md` | Full skill catalog |
| `docs/ai/AI_ARCHITECTURE.md` | AI system architecture |
| `docs/ai/AI_WORKFLOW.md` | Development workflow |
| `taste/taste-registry.yaml` | Taste bundles |
| `quality/quality-registry.yaml` | Quality bundles |
| `security/security-registry.yaml` | Security bundles |
