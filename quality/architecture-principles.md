# Architecture Excellence Principles — Teknovo Impeccable Architect

> **Authority**: Pillar 2 — Chief Architect; ADRs (`docs/adr/**`)  
> **Layer order**: Feature → API → Service → Repository → Database (from `AGENTS.md`)

---

## Purpose

Teknovo is a multi-domain school ERP on a monorepo with strict bounded contexts. Architecture excellence means **clear ownership**, **predictable boundaries**, and **maintainable evolution** — not clever abstractions.
...
- Cross-domain reads via API or read model — not direct repository import
- Domain events documented for side effects (e.g., `StudentEnrolled` → Finance creates billing plan)
...
**Question**: What is the public contract — and what stays internal?

**Standard stack**:

```text
HTTP Controller  →  Zod validation  →  Service  →  Repository  →  PostgreSQL
```

| Rule | Requirement |
|------|-------------|
| Export surface | Services only — repositories are module-private |
...
**Question**: Who may write this row — and how are deletes handled?

| Rule | Standard |
|------|----------|
| Primary keys | UUID v7 only — no auto-increment |
...
| Read models | Reporting aggregates — no write-back |
...
```text
┌─────────────┐
│   Feature   │  User story, PRD, acceptance criteria
└──────┬──────┘
       ▼
┌─────────────┐
│     API     │  Routes, Zod schemas, RBAC guards, OpenAPI
└──────┬──────┘
       ▼
┌─────────────┐
...
└──────┬──────┘
       ▼
┌─────────────┐
│ Repository  │  Drizzle queries, soft-delete filters (private)
└──────┬──────┘
       ▼
┌─────────────┐
│  Database   │  PostgreSQL 17, indexes, constraints
└─────────────┘
```
...
| Pattern | Example |
|---------|---------|
| Domain event | `ApplicantAccepted` → Academic creates student record |
...
**Related**: `.agents/skills/teknovo-chief-architect/SKILL.md`, `quality/quality-gates.md`, `quality/review-checklist.md`
