...
REPORT_DIR="${REPO_ROOT}/.bootstrap/reports"
...
verify_registries() {
  ...
  for f in skill-registry.yaml agent-registry.yaml mcp-registry.yaml; do
    check "registry/${f}" "test -f ${REPO_ROOT}/registry/${f}"
  done
}
...
verify_docs() {
  ...
  check "preflight.sh" "test -x ${REPO_ROOT}/bootstrap/preflight.sh"
  warn_check "GPU detection lib" "test -f ${REPO_ROOT}/bootstrap/lib/gpu.sh"
}
