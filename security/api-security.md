# API Security — Teknovo Security System

> **Scope**: Authentication, authorization, input/output validation, rate limiting, error handling, API versioning  
> **Aligns with**: `docs/standards/api/api-contract.md`, `.agents/skills/teknovo-api-architect/SKILL.md`  
...
Teknovo APIs serve subdomain-isolated apps (`erp.*`, `ppdb.*`, `finance.*`, `cbt.*`, `api.*`). All mutation and sensitive read traffic flows through the **Controller → Service → Repository** stack — never expose repository or raw SQL to HTTP.

```text
Client (Nuxt) → Cloudflare (TLS, WAF, rate limit) → API (auth, RBAC, Zod) → Service → Repository → PostgreSQL
```
...
**Subdomain cookies**: Scope cookies per subdomain (`erp.domain.sch.id`) — do not share session across unrelated subdomains without explicit SSO design.
...
1. `AuthGuard` — valid session/JWT
2. `PermissionsGuard` — `domain.resource.action`
3. Service ownership filter — tenancy and IDOR prevention

See `security/rbac-security.md` for full RBAC rules.
...
| Pattern | Risk | Fix |
|---------|------|-----|
| Raw `req.body` to service | Injection, type confusion | Zod parse → typed DTO |
...
const CreateApplicantSchema = z.object({
  ...
  // school_id from route/host context — NOT from body
});
```
...
| Pagination metadata | `cursor`, `hasMore` — never full count on huge tables |
...
| Rule | Detail |
|------|--------|
| Path version | `/api/v1/...` — current standard |
...
**Security note**: Do not maintain unauthenticated legacy versions alongside v1 — sunset or gate behind auth.
...
- Cloudflare: `security/cloudflare-security.md`
