...
export function createMcpServer(tools) {
  const server = new Server(
    { name: 'teknovo-github-mcp', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
  ...
  if (!tool) {
    return {
      ...
      content: [
        {
          type: 'text',
          text: JSON.stringify({ success: false, error: `Unknown tool: ${toolName}` }),
        },
      ],
    };
  }
...
async function main() {
  ...
  logger.info('Starting teknovo-github-mcp', { toolCount: tools.size });
