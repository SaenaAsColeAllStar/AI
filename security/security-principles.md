# Security Principles — Teknovo Security System
...
> **Tone**: Principal Security Architect — proactive risk identification, secure defaults, explicit permissions
...
Security is not a post-ship checkbox. A feature that bypasses security review is **blocked** — regardless of product value or engineering quality.
...
Aligned with defense-in-depth and secure-by-default practice (including public guidance such as Anthropic's cybersecurity skills philosophy): **mistakes happen** — design systems that fail safely.
...
|--------|----------|--------------|
| RBAC & authorization | `security/rbac-security.md` | New routes, menus, permissions, roles |
| API & auth | `security/api-security.md` | Endpoints, webhooks, integrations |
| Database & data | `security/database-security.md` | Migrations, schemas, queries |
| Cloudflare & infra | `security/cloudflare-security.md` | Deploy, DNS, Workers, D1, R2, tunnels |
| AI workstation | `security/ai-agent-security.md` | Agent sessions, MCP, automation, commits |
| Review checklist | `security/review-checklist.md` | Pre-PR, pre-deploy, security audit |
| Gates | `security/security-gates.md` | Before implementation, deploy, production |

**Skill**: `.agents/skills/teknovo-security-review/SKILL.md` — tactical auth/CORS/JWT/rate-limit controls  
**Agent**: `agents/security-reviewer.md` — formal Security Review with verdict
...
```text
Discovery → Planning → Taste (scope) → Architecture (Pillar 2)
    ↓
SECURITY REVIEW ← mandatory before implementation
    ↓
Implementation → Impeccable Quality → Ship (Pillar 3)
```

Detail: `security/security-gates.md`, `docs/ai/AI_SECURITY_SYSTEM.md`
...
| **Impeccable Quality** | Quality gate includes Security Review (`quality/quality-gates.md`) |
...
- Runtime: `python ai-agent/runtime/load-memory.py --include-security`
- Architecture: `memory/architecture-decisions.md` — Cloudflare, RBAC
