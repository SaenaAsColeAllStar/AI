#!/usr/bin/env python3
"""Build registry/skill-registry.yaml from consolidated Teknovo sources."""
...
CONFLICT_RESOLUTION = {
    "document_priority": [
        ...
        ".agents/registry.yaml (legacy compatibility)",
...
EXTRA_SKILLS: dict[str, dict[str, Any]] = {
    "agents-md": {
        ...
        "source": "foundation",
        "path": "AGENTS.md",
        "description": "Master agent contract — identity, workflow, Three Pillars, layer precedence",
        "depends_on": [],
    ...
    "quality-review-principles": {
        ...
        "priority": "high",
        "source": "quality",
        "path": "quality/review-principles.md",
    ...
    "security-principles": {
        ...
        "priority": "critical",
        "source": "security",
        "path": "security/security-principles.md",
        "description": "Master security principles — least privilege, defense in depth",
        "depends_on": ["agents-md"],
    ...
    "security-rbac-security": {
        ...
        "priority": "critical",
        "source": "security",
        "path": "security/rbac-security.md",
    ...
    "security-database-security": {
        ...
        "priority": "high",
        "source": "security",
        "path": "security/database-security.md",
    ...
    "security-cloudflare-security": {
        ...
        "priority": "high",
        "source": "security",
        "path": "security/cloudflare-security.md",
    ...
    "security-ai-agent-security": {
        ...
        "priority": "high",
        "source": "security",
        "path": "security/ai-agent-security.md",
    ...
    "security-review-checklist": {
        ...
        "priority": "high",
        "source": "security",
        "path": "security/review-checklist.md",
    ...
    "security-gates": {
        ...
        "priority": "critical",
        "source": "security",
        "path": "security/security-gates.md",
        "description": "Mandatory security gates — pre-implementation, pre-deploy, pre-production",
        "depends_on": ["security-principles"],
    ...
    "security-reviewer": {
        ...
        "description": "Security review agent — risk level, attack surface, APPROVE/BLOCK verdict",
    ...
    "requirement-clarifier": {
        ...
        "description": "Requirements auditor — ambiguity, conflicts, missing criteria before planning",
    ...
    "context-builder": {
        ...
        "description": "Context architect — verifies ADR/PRD/standards loaded before decisions",
    ...
    "differential-reviewer": {
        ...
        "description": "[PLANNED] Diff-focused review — lockfile, migration, RBAC delta analysis",
    ...
    "assurance-principles": {
        ...
        "priority": "high",
        "source": "assurance",
        "path": "assurance/assurance-principles.md",
        "description": "Assurance philosophy — verify assumptions, evidence over narrative",
        "depends_on": ["agents-md"],
    ...
    "assurance-review-workflow": {
        ...
        "priority": "high",
        "source": "assurance",
        "path": "assurance/review-workflow.md",
    ...
    "assurance-risk-analysis": {
        ...
        "priority": "medium",
        "source": "assurance",
        "path": "assurance/risk-analysis.md",
    ...
    "assurance-decision-validation": {
        ...
        "priority": "medium",
        "source": "assurance",
        "path": "assurance/decision-validation.md",
    ...
    "assurance-insecure-defaults": {
        ...
        "priority": "high",
        "source": "assurance",
        "path": "assurance/insecure-defaults.md",
    ...
    "assurance-dependency-review": {
        ...
        "priority": "high",
        "source": "assurance",
        "path": "assurance/dependency-review.md",
    ...
    "assurance-static-analysis": {
        ...
        "priority": "medium",
        "source": "assurance",
        "path": "assurance/static-analysis.md",
...
MEMORY_SKILLS = {
    "memory-project-context": {"path": "memory/project-context.md", "layer": "memory", "priority": "critical",
        "description": "Master project context — agent identity, workflow, constraints", "autoload": True,
        "activate_when": ["session start", "bootstrap"]},
    "memory-repository-map": {"path": "memory/repository-map.md", "layer": "memory", "priority": "high",
    ...
    "memory-product-context": {"path": "memory/product-context.md", "layer": "memory", "priority": "high",
    ...
    "memory-domain-knowledge": {"path": "memory/domain-knowledge.md", "layer": "memory", "priority": "high",
        "description": "Bounded contexts, schemas, domain events", "autoload": True,
        "activate_when": ["domain", "bounded context", "domain event"]},
    "memory-architecture-decisions": {"path": "memory/architecture-decisions.md", "layer": "memory", "priority": "high",
        "description": "ADR summaries — monorepo, Cloudflare, Nuxt, RBAC", "autoload": True,
        "activate_when": ["ADR", "architecture decision", "why we chose"]},
    "memory-coding-standards": {"path": "memory/coding-standards.md", "layer": "memory", "priority": "critical",
    ...
    "memory-ui-ux-rules": {"path": "memory/ui-ux-rules.md", "layer": "memory", "priority": "high",
        "description": "Color tokens, typography, sidebar, page states", "autoload": True,
        "activate_when": ["design token", "sidebar", "page state"]},
    "memory-lessons-learned": {"path": "memory/lessons-learned.md", "layer": "memory", "priority": "medium",
...
def infer_source(skill_id: str, path: str) -> str:
    ...
    if path.startswith("agents/"):
        ...
        if skill_id == "security-reviewer":
            return "security"
        return "assurance"
    if path.startswith(".agents/skills/superpowers/"):
        return "superpowers"
    if path.startswith(".agents/skills/gstack/"):
        return "gstack"
    if path.startswith(".agents/skills/teknovo"):
        return "teknovo"
    if path in ("AGENTS.md", ".agents/AGENTS.md"):
        return "foundation"
    return "teknovo"
...
def build_skills() -> dict[str, dict]:
    ...
    for sid, entry in flat.items():
        ...
        skills[sid] = {
            ...
            "conflicts": [],
            "description": entry.get("description", f"Skill {sid} — see path"),
...
def main() -> None:
    ...
    registry = {
        ...
        "metadata": {
            ...
            "description": "Central orchestration layer — single source of truth for skill discovery, layers, priorities, activation, and conflicts",
            "last_updated": date.today().isoformat(),
            "maintainer": "Teknovo AI SuperStack",
            "legacy_registry": ".agents/registry.yaml",
            "governance_doc": "docs/ai/skill-governance.md",
            "inventory_doc": "docs/ai/skill-inventory.md",
            "agent_registry": "registry/agent-registry.yaml",
            "mcp_registry": "registry/mcp-registry.yaml",
        },
        "layers": LAYERS,
        "priority_levels": {
        ...
        "sub_registries": {
            "taste": {"path": "taste/taste-registry.yaml", "prefix": "taste-", "note": "Taste artifacts mirrored here; bundles in sub-registry"},
            "quality": {"path": "quality/quality-registry.yaml", "prefix": "quality-", "note": "Impeccable quality artifacts"},
            "security": {"path": "security/security-registry.yaml", "prefix": "security-", "note": "Security layer artifacts"},
            "memory": {"path": "memory/memory-registry.yaml", "prefix": "memory-", "note": "Claude-Mem long-term artifacts"},
            "assurance": {"path": "assurance/", "prefix": "assurance-", "note": "Assurance artifacts; assurance-registry.yaml [PLANNED]"},
        },
        "autoload": [s for s, v in skills.items() if v.get("autoload")],
        "bundles": {
            "session-bootstrap": {
                "description": "Default session — foundation + memory + autoload skills",
                "skills": ["agents-md", "memory-project-context", "memory-repository-map"],
            },
            "pre-feature": {
                "description": "Before feature work — taste + assurance + pillar 1",
                "skills": ["taste-gates", "taste-reviewer", "requirement-clarifier", "teknovo-chief-product-designer"],
            },
            "pre-implementation": {
                "description": "Before code — architecture + security + context",
                "skills": ["teknovo-chief-architect", "context-builder", "security-reviewer", "security-gates"],
            },
            "pre-ship": {
                "description": "Before merge/deploy — quality + security + QA",
    ...
    header = (
        "# Teknovo Skill Registry — Central Orchestration Layer\n"
        "# Single source of truth. Legacy: .agents/registry.yaml (backward compatible)\n"
        "# Schema: id, layer, priority, source, path, depends_on, activate_when, conflicts, description\n"
        "# Extend: add skill block under skills: — run ai-agent/runtime/load-skills.py --validate\n\n"
    )
