...
generate_skill_registry() {
  ...
  info "Generating registry/skill-registry.yaml from .agents/skills..."
  local skill_count
  skill_count="$(find "${REPO_ROOT}/.agents/skills" -name 'SKILL.md' 2>/dev/null | wc -l | tr -d ' ')"

  cat > "${out}" << EOF
# Teknovo AI Workstation — Skill Registry Index
# Aggregates .agents/skills for bootstrap verification and tooling.
# Canonical skill definitions remain in .agents/registry.yaml

version: "1.0"
last_updated: "$(date +%Y-%m-%d)"
generated_by: bootstrap/build-registries.sh

source:
  canonical: .agents/registry.yaml
...
EOF
  ...
  find "${REPO_ROOT}/.agents/skills" -name 'SKILL.md' 2>/dev/null | sort | while read -r skill_file; do
    local rel dir name
...
generate_agent_registry() {
  ...
  info "Generating registry/agent-registry.yaml from agents/..."
  cat > "${out}" << EOF
...
generate_mcp_registry() {
  ...
  info "Generating registry/mcp-registry.yaml..."
  cat > "${out}" << 'EOF'
...
servers:
  ...
  cloudflare:
    path: mcp/cloudflare
    config_template: mcp/cloudflare/config.template.json
    status: placeholder
    required_env:
...
validate_registries() {
  local files=(
    ...
    "${REPO_ROOT}/.agents/registry.yaml"
    "${REPO_ROOT}/memory/memory-registry.yaml"
    "${REPO_ROOT}/quality/quality-registry.yaml"
    "${REPO_ROOT}/taste/taste-registry.yaml"
    "${REPO_ROOT}/security/security-registry.yaml"
    "${REPO_ROOT}/assurance/assurance-registry.yaml"
  )
