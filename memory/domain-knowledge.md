# Domain Knowledge — Teknovo School ERP

> **Source**: `docs/ai/repository-analysis.md`, domain skills in `.agents/skills/teknovo-{finance,ppdb,cbt,academic,communication,reporting}/`  
...
| PPDB | `ppdb.domain.sch.id` | Student admission — applicants, registration, verification, selection |
| ERP | `erp.domain.sch.id` | Core school management — academics, classes, grading, attendance |
| CBT | `cbt.domain.sch.id` | Computer-based testing — exams, question banks, proctoring |
| Finance | `finance.domain.sch.id` | Billing plans, student payments, receipts, cash books |
| WA | `wa.domain.sch.id` | WhatsApp notifications — templates, campaigns, delivery logs |
...
**Skill**: `teknovo-academic` · **Schema**: `academic` · **Subdomain**: `erp.domain.sch.id`
...
**Skill**: `teknovo-ppdb` · **Schema**: `ppdb` · **Subdomain**: `ppdb.domain.sch.id`

**Purpose**: Applicant registration, document verification, selection, announcement, re-registration.

**Principles**:
- **Applicant First** — entity is Applicant, not Student, until acceptance
- Student created only on `ppdb.applicant.accepted` event → Student Domain handler
...
**Key Event**: `ppdb.applicant.accepted` → triggers Student creation

---

### 3. CBT Domain (Computer Based Testing)

**Skill**: `teknovo-cbt` · **Schema**: `cbt` · **Subdomain**: `cbt.domain.sch.id`
...
- Published results immutable — corrections via official appeal workflow
...
**Skill**: `teknovo-finance` · **Schema**: `finance` · **Subdomain**: `finance.domain.sch.id`

**Purpose**: Fee types, billing rules, student bills, payments, receipts, cash books, arrears (tunggakan).

**Principles**:
- **Financial history is immutable** — corrections via reversal/adjustment entries
- Posted transactions never hard-deleted; soft delete only for drafts
- Student owned by Student Domain — reference by ID only
- PPDB registration payments owned by PPDB lifecycle first

**Payment states**: `draft → pending → paid | failed | cancelled`
...
**Skill**: `teknovo-communication` · **Schema**: `wa` · **Subdomain**: `wa.domain.sch.id` · **Port**: 4001

**Purpose**: WhatsApp devices, templates, campaigns, message delivery, notification queues.

**Principles**:
- **Event driven** — react to domain events, no polling other domains' databases
- **No business ownership** — resolve contacts via read API or event payload IDs
- **Template first** — every outbound message uses approved template
...
**Skill**: `teknovo-reporting` · **Schema**: read models across domains

**Purpose**: Dashboards, analytics, PDF export, scheduled reports.

**Principles**:
- **Read-only** — no write-back to source domains
...
**Principle**: Other domains reference Student by ID only — never duplicate canonical records.
...
| Applicant | PPDB Domain | — until accepted |
...
Reference: `docs/standards/rbac/rbac-standard.md`, `docs/security/rbac-matrix.md`
