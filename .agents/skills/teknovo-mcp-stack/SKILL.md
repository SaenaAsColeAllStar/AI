...
description: Orchestrate Teknovo MCP servers — Cloudflare, GitHub, filesystem, git, PostgreSQL, and Qdrant — for deploy, dev, and investigation workflows.
---
...
```text
1. filesystem/read_file + shell → verify build config
2. shell → npm run build && npm test
3. cloudflare/pages_list_projects → check project exists
4. cloudflare/pages_create_project OR pages_deploy
5. cloudflare/domain_attach → cloudflare/domain_verify
6. github/pr_create → ship PR
```

### PR workflow

```text
1. git/git_status + git/git_diff → review changes
2. git/git_commit → stage and commit (user-requested only)
3. git/git_push → push branch (no force)
4. github/pr_create → open PR
5. github/workflow_list → github/workflow_dispatch (if needed)
```

### Investigation workflow

```text
1. postgres/list_tables → postgres/describe_table
2. postgres/query → read-only SELECT / EXPLAIN
3. qdrant/search → semantic doc retrieval
```
...
### teknovo-cloudflare-mcp (`mcp/cloudflare/`)
...
See `.agents/skills/teknovo-cloudflare-stack/SKILL.md` for deploy sequence.
...
| Force push via git MCP | Blocked by design — explicit user + different tool |
