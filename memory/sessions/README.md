...
| Field | Purpose |
|-------|---------|
| `memory/sessions/` | Per-session markdown logs |
| `memory/lessons-learned.md` | Cross-session operational lessons |
| `memory/memory-registry.yaml` | Index of all memory artifacts |
...
`ai-agent/runtime/load-memory.py` can include recent session summaries when invoked with:
...
- `memory/lessons-learned.md` — operational failures and fixes
- `memory/project-context.md` — static project context
