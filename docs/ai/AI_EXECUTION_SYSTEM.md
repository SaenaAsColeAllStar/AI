...
> **Bootstrap**: `.agents/EXECUTION.md` (read after `AGENTS.md`)  
> **Registry**: `execution/execution-registry.yaml`
...
| **Lock file** | `bootstrap/install.lock.yaml` |
...
3. Point to `bootstrap/install-model.sh` and `AI_RUNTIME.md`.
...
| Rule | Requirement |
|------|-------------|
| Tool exists | **Use the tool** — never describe what could be executed |
| Instructions | **Forbidden** — do not tell the user to run commands you can run |
| Plans | **Forbidden** when Principal Architect requests direct execution |
| TODO lists | **Forbidden** in execution-only sessions |
| Next steps | **Forbidden** — complete the work, then report status |
...
Detail: `execution/failure-recovery.md`

---

## Self-Healing Mode

When a command returns **exit code ≠ 0**:
...
Registry: `execution/execution-registry.yaml` → `retry_policies.self_healing`
...
Detail: `execution/branch-policy.md`
...
2. If branch is `main` or `master` → create and checkout a feature branch.
...
Deployment workflow: `execution/deployment-mode.md`
...
```text
build → test → fix (self-heal) → deploy → verify DNS → verify HTTPS
```
...
Detail: `execution/deployment-mode.md`
...
| MCP registration | `registry/mcp-registry.yaml` if MCP added |
| Configuration | Env / config files consistent |

Fix issues automatically. Do not report "done" with failing build or tests.

Cross-ref: `quality/quality-gates.md`, `quality/self-critique.md`
...
| Action | Policy |
|--------|--------|
| Push | Never automatic — user must request |
...
Nothing else — no plans, no next steps, no TODO lists.
...
| Artifact | Path |
|----------|------|
| Agent bootstrap | `.agents/EXECUTION.md` |
| Registry | `execution/execution-registry.yaml` |
| Failure recovery | `execution/failure-recovery.md` |
| Branch policy | `execution/branch-policy.md` |
| Deployment mode | `execution/deployment-mode.md` |
| Master rules | `AGENTS.md` § Execution System V2 |
| Model lock | `bootstrap/install.lock.yaml` |
...
| Principle | Rule |
|-----------|------|
| Model | `qwen3:32b` only (see `bootstrap/install.lock.yaml`) |
| Mode | Execute with tools — do not instruct |
| Failure | Recover automatically — max 10 retries |
| Branch | Feature branches only — never commit to `main` |
| Secrets | Host secret store only — never in repo |
| Deploy | Build → test → fix → deploy → verify |
| Done | Build + tests + validation pass — then report status |
