...
python ai-agent/runtime/load-skills.py --validate

# List skills per layer
python ai-agent/runtime/load-skills.py --list-layers

# Resolve skills by user intent keywords
python ai-agent/runtime/load-skills.py --trigger "RBAC permission"

# Load a workflow bundle
python ai-agent/runtime/load-skills.py --bundle pre-implementation

# Load autoload set
python ai-agent/runtime/load-skills.py --autoload --format json
```

Combine with memory/taste/quality loaders:
...
`.agents/registry.yaml` remains for **backward compatibility**. It declares:

```yaml
orchestration:
  canonical: registry/skill-registry.yaml
```

New skills must be added to `registry/skill-registry.yaml` first. Legacy autoload IDs should stay in sync with canonical `autoload` list.
...
See `docs/ai/skill-governance.md` for conflict rules and how to add skills.
...
To add skill #98+, copy an existing block in `skill-registry.yaml` → `skills`, fill all required fields, validate, update `docs/ai/skill-inventory.md`.
...
- Inventory: `docs/ai/skill-inventory.md`
- Governance: `docs/ai/skill-governance.md`
