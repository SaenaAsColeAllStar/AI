...
Loads all memory artifacts from memory/ and merges into runtime context.
Runnable as CLI script and importable as module.

Usage:
    python ai-agent/runtime/load-memory.py
    python ai-agent/runtime/load-memory.py --format json
    python ai-agent/runtime/load-memory.py --include-sessions --session-limit 3
    python ai-agent/runtime/load-memory.py --keys architecture-decisions,ui-ux-rules
    python ai-agent/runtime/load-memory.py --include-quality
    python ai-agent/runtime/load-memory.py --include-quality --quality-bundle pre-ship
    python ai-agent/runtime/load-memory.py --include-security
    python ai-agent/runtime/load-memory.py --include-security --security-bundle pre-api
    ...
    python ai-agent/runtime/load-memory.py --include-assurance
    python ai-agent/runtime/load-memory.py --include-assurance --assurance-bundle pre-implementation
    python ai-agent/runtime/load-memory.py --include-taste --include-assurance --include-security --assurance-bundle pre-implementation
"""
...
def find_repo_root(start: Path | None = None) -> Path:
    """Walk up from start (or this file) to find repo root containing memory/."""
    ...
    for candidate in [current, *current.parents]:
        if (candidate / "memory" / "memory-registry.yaml").exists():
            return candidate
        if (candidate / "AGENTS.md").exists() and (candidate / "memory").is_dir():
            return candidate
...
def load_taste_registry(repo_root: Path) -> dict[str, Any]:
    registry_path = repo_root / "taste" / "taste-registry.yaml"
    if not registry_path.exists():
...
def load_taste_artifacts(
    ...
    """Load taste artifacts by key from taste/taste-registry.yaml."""
    registry = load_taste_registry(repo_root)
    artifacts_cfg: dict[str, Any] = registry.get("artifacts", {})
    ...
    for key in sorted(keys, key=lambda k: artifacts_cfg.get(k, {}).get("priority", 99)):
        ...
        rel_path = cfg.get("path", f"taste/{key}.md")
...
def load_assurance_registry(repo_root: Path) -> dict[str, Any]:
    registry_path = repo_root / "assurance" / "assurance-registry.yaml"
    if not registry_path.exists():
...
def load_assurance_artifacts(
    ...
    """Load assurance artifacts by key from assurance/assurance-registry.yaml."""
    registry = load_assurance_registry(repo_root)
    artifacts_cfg: dict[str, Any] = registry.get("artifacts", {})
    loaded: dict[str, MemoryArtifact] = {}
    warnings: list[str] = []
    ...
    for key in sorted(keys, key=lambda k: artifacts_cfg.get(k, {}).get("priority", 99)):
        cfg = artifacts_cfg.get(key, {})
        rel_path = cfg.get("path", f"assurance/{key}.md")
        path = repo_root / rel_path
...
def load_quality_registry(repo_root: Path) -> dict[str, Any]:
    registry_path = repo_root / "quality" / "quality-registry.yaml"
    if not registry_path.exists():
        return {"artifacts": {}, "bundles": {}}
...
def load_quality_artifacts(
    ...
    """Load quality artifacts by key from quality/quality-registry.yaml."""
    registry = load_quality_registry(repo_root)
    artifacts_cfg: dict[str, Any] = registry.get("artifacts", {})
    loaded: dict[str, MemoryArtifact] = {}
    warnings: list[str] = []
    ...
    for key in sorted(keys, key=lambda k: artifacts_cfg.get(k, {}).get("priority", 99)):
        cfg = artifacts_cfg.get(key, {})
        rel_path = cfg.get("path", f"quality/{key}.md")
        path = repo_root / rel_path
...
def load_security_registry(repo_root: Path) -> dict[str, Any]:
    registry_path = repo_root / "security" / "security-registry.yaml"
    if not registry_path.exists():
        return {"artifacts": {}, "bundles": {}}
...
def load_security_artifacts(
    ...
    """Load security artifacts by key from security/security-registry.yaml."""
    registry = load_security_registry(repo_root)
    artifacts_cfg: dict[str, Any] = registry.get("artifacts", {})
    loaded: dict[str, MemoryArtifact] = {}
    warnings: list[str] = []
    ...
    for key in sorted(keys, key=lambda k: artifacts_cfg.get(k, {}).get("priority", 99)):
        cfg = artifacts_cfg.get(key, {})
        rel_path = cfg.get("path", f"security/{key}.md")
        path = repo_root / rel_path
...
def load_memory(
    ...
    for key in sorted(target_keys, key=lambda k: artifacts_cfg.get(k, {}).get("priority", 99)):
        cfg = artifacts_cfg.get(key, {})
        rel_path = cfg.get("path", f"memory/{key}.md")
        path = root / rel_path
    ...
    if include_sessions or (keys and "sessions" in keys):
        ...
        sessions_dir = root / sessions_cfg.get("path", "memory/sessions/")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--include-taste",
        action="store_true",
        help="Include taste artifacts with autoload=true from taste/taste-registry.yaml",
    )
    parser.add_argument(
        "--taste-bundle",
        type=str,
    ...
    parser.add_argument(
        "--taste-only",
        action="store_true",
        help="Load only taste artifacts (no memory/ artifacts)",
    )
    parser.add_argument(
        "--include-quality",
        action="store_true",
        help="Include quality artifacts with autoload=true from quality/quality-registry.yaml",
    )
    parser.add_argument(
        "--quality-bundle",
        type=str,
    ...
    parser.add_argument(
        "--quality-only",
        action="store_true",
        help="Load only quality artifacts (no memory/ artifacts)",
    )
    parser.add_argument(
        "--include-security",
        action="store_true",
        help="Include security artifacts from security/security-registry.yaml",
    )
    parser.add_argument(
        "--security-bundle",
        type=str,
    ...
    parser.add_argument(
        "--security-only",
        action="store_true",
        help="Load only security artifacts (no memory/ artifacts)",
    )
    parser.add_argument(
        "--assurance-only",
        action="store_true",
        help="Load only assurance artifacts (no memory/ artifacts)",
    )
    parser.add_argument(
        "--include-assurance",
        action="store_true",
        help="Include assurance artifacts with autoload=true from assurance/assurance-registry.yaml",
    )
    parser.add_argument(
        "--assurance-bundle",
        type=str,
