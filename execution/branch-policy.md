# Branch Policy — Execution System V2

> **Registry**: `execution/execution-registry.yaml` → `branch_policy`  
...
| Branch | Status |
|--------|--------|
| `main` | Protected — AI infrastructure only via reviewed merge |
...
Infra changes still use **feature branches** → commit → PR → human merge.
...
Skill: `.agents/skills/superpowers/using-git-worktrees/SKILL.md`
...
- Format: `feat(scope): description` (see `execution/execution-registry.yaml` → `git_rules`).
- Stage only relevant files — never secrets or `.env`.
...
| Doc | Purpose |
|-----|---------|
| `.agents/EXECUTION.md` | Bootstrap |
| `docs/ai/AI_EXECUTION_SYSTEM.md` | Full spec |
| `AGENTS.md` | Master workflow |
| `execution/failure-recovery.md` | Retry on git hook failures |
