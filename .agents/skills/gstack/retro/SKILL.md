...
2. **What didn't go well?** — friction points, wasted effort
3. **What surprised us?** — unexpected complexity, doc gaps
4. **What would we do differently?** — process improvements
5. **What skills/docs need updating?** — registry or standard gaps
...
- Time in each phase: Discovery [X] → Planning [X] → Code [X] → QA [X]
- Review rounds: X
...
- [ ] Update `.agents/skills/<skill>/SKILL.md` with [change]
...
1. Update the relevant `SKILL.md` in `.agents/skills/`
2. Update trigger in `.agents/registry.yaml` if needed
3. Update `docs/ai/AI_SKILLS_CATALOG.md`
4. Consider adding to `docs/ai/AI_ROADMAP.md` if major gap
...
- Be honest — retros are for improvement, not blame
