...
Resolves skills from registry/skill-registry.yaml by trigger keywords, bundles,
or explicit skill IDs. Complements ai-agent/runtime/load-memory.py.
...
Usage:
    ...
    python ai-agent/runtime/load-skills.py --ids teknovo-rbac-architect,security-rbac-security
...
def find_repo_root(start: Path | None = None) -> Path:
    ...
    for candidate in [current, *current.parents]:
        if (candidate / "registry" / "skill-registry.yaml").exists():
            return candidate
...
def load_skill_registry(repo_root: Path) -> dict[str, Any]:
    path = repo_root / "registry" / "skill-registry.yaml"
    if not path.exists():
...
def load_skills(
    ...
    for sid, reason in id_reasons.items():
        ...
        if status == "planned" and not path.exists():
            ...
            content = f"# [PLANNED] {sid}\n\nSee registry/skill-registry.yaml and docs/ai/skill-governance.md\n"
            err = None
...
def validate_registry(repo_root: Path) -> list[str]:
    ...
    if not skills:
        errors.append("No skills defined in registry/skill-registry.yaml")
        return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not any([args.ids, args.bundle, args.trigger, args.autoload]):
        parser.error("Specify --ids, --bundle, --trigger, --autoload, --validate, or --list-layers")
