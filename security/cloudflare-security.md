# Cloudflare Security — Teknovo Security System

> **Scope**: Workers, D1, R2, DNS, tunnels, secrets, environment variables  
> **Source**: `memory/architecture-decisions.md`, `.agents/skills/teknovo-cloudflare-stack/SKILL.md`, `AI_DEPLOY.md`  
...
Reference decision record: `memory/architecture-decisions.md` — Cloudflare Edge Infrastructure.

---

## Subdomain Isolation

Each ERP domain on dedicated subdomain — separate cookie scope and CORS policy:
...
| Rule | Detail |
|------|--------|
| Origin bind | Services on `127.0.0.1` only — never `0.0.0.0` for DB/API |
| Ingress rules | Explicit hostname → local port mapping |
| Tunnel token | Stored as secret — not in git |
...
| Secrets | `wrangler secret put` — never in `wrangler.toml` committed |
...
Optional Workers/D1 for lightweight edge tasks — **not** a replacement for authoritative PostgreSQL without architecture approval.
...
AI agents: **no autonomous production DNS changes** without human approval (`security/ai-agent-security.md`).
...
- AI agent deploy limits: `security/ai-agent-security.md`
- API CORS/headers: `security/api-security.md`
- Gates: `security/security-gates.md`
