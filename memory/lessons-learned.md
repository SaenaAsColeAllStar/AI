...
> **Source**: `AI_DEPLOY.md`, `docs/ai/repository-analysis.md`, `docs/ai/AI_ROADMAP.md`, integration gaps  
...
**Source**: `docs/ai/repository-analysis.md` § Integration Gaps

**Symptoms**: Agent ignores Three Pillars gates; missing domain skills

**Cause**: `.agents/` not symlinked/copied from AI repo into Teknovo-V2
...
**Source**: `docs/ai/AI_ROADMAP.md` M2
...
**Source**: `docs/ai/repository-analysis.md`, `AI_ROADMAP.md` M3

**Status**: Planned Q4 2026 — OpenCode config with skill bootstrap hook, Redis session cache

**Workaround**: Manually read `AGENTS.md` and autoload skills from registry each session. Use `ai-agent/runtime/load-memory.py` for memory context injection.
...
> **Qdrant is not part of the Teknovo AI SuperStack.** This repo uses markdown memory files (`memory/`) and skill registry — not vector databases.
...
**Teknovo approach**: Use `memory/memory-registry.yaml` + `load-memory.py` for structured context, not Qdrant.
...
Also add session summary to `memory/sessions/YYYY-MM-DD-<topic>.md`.
...
| Qdrant vector memory | `memory/` + `load-memory.py` |
